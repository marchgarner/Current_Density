...
def identify_and_align(molecule, align1, align2):
...
def plot_current(jx,jy,jz,x,y,z,savename,s,amp,co,path, align1, align2):
    ...
    cyl_colorlist=[]
    for n in np.arange(n_bins[0]):
        cyl_colorlist.append(list(cm(n))[:-1])	
    print(cyl_colorlist)      
    au2A = 0.529177249
    x = x[::s]*au2A
    y = y[::s]*au2A
    ...
    with open(savename+"_zcolor.spt", "w") as text_file:
        text_file.write('load "file:central_region.xyz" \n'.format(path))
        text_file.write('write central_region2.xyz \n'.format(path))
        text_file.write('load "file:central_region2.xyz" \n'.format(path))
        a = 0
        for ix, x2 in enumerate(x):
            for iy, y2 in enumerate(y):
                for iz, z2 in enumerate(z):
                    ...
                    if norm2 > co and z2 > refcoord1[2] and z2 < refcoord2[2]: #and norm < co*1000:
